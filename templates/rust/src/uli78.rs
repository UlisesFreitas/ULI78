...
// The wrapper functions are designed to be similar to the usual ULI-78 api.
pub mod sys {
    #[derive(Default)]
    #[repr(C)]
    ...
    extern "C" {
        ...
        // `remap` is not yet implemented by the ULI-78 WASM runtime, so for now its type is a raw i32.
        pub fn map(
            x: i32,
            y: i32,
            w: i32,
...
// "use uli78::*" causes this to shadow std::print, but that isn't useful here anyway.
#[macro_export]
macro_rules! print {
    ($text: literal, $($args: expr), *) => {
        $crate::uli78::print_raw(concat!($text, "\0"), $($args), *);
    };
    ($text: expr, $($args: expr), *) => {
        $crate::uli78::print_alloc($text, $($args), *);
    };
}
...
// NOTE: "use uli78::*" causes this to shadow std::print, but that isn't useful here anyway.
#[macro_export]
macro_rules! font {
    ($text: literal, $($args: expr), *) => {
        $crate::uli78::font_raw(concat!($text, "\0"), $($args), *);
    };
    ($text: expr, $($args: expr), *) => {
        $crate::uli78::font_alloc($text, $($args), *);
    };
}
...
macro_rules! trace {
    ($text: literal, $color: expr) => {
        unsafe { crate::uli78::sys::trace(concat!($text, "\0").as_ptr(), $color) }
    };
    ($text: expr, $color: expr) => {
        crate::uli78::trace_alloc($text, $color);
    };
}
