...
- ```wasmdemo.wasmp``` - ULI-78 Wasm 'script' file. Note the embedded game assets data at the end of the file.
...
To build a ULI-78 cartridge, first build the Wasm file, then build the
cartridge file:
...
## Additional Notes
ULI-78 API functions that merely duplicate standard library functionality are not imported. Please use the `memcpy` and `memset` functions provided by the C standard library instead.