...
```
mkdir -p ~/Applications/uli78dev.app/Contents/{MacOS,Resources}
cp -f macosx/uli78.plist ~/Applications/uli78dev.app/Contents/Info.plist
cp -f macosx/uli78.icns ~/Applications/uli78dev.app/Contents/Resources
cat > ~/Applications/uli78dev.app/Contents/MacOS/uli78 <<EOF
#!/bin/sh
exec /Users/uli78/projects/ULI-78/build/bin/uli78 --skip >/dev/null
EOF
chmod +x ~/Applications/uli78dev.app/Contents/MacOS/uli78
```
Make sure to update the absolute path to the uli78 binary in the script, or
...
## Linux
- To make an executable file `./uli78` without installation run `make` and locate the output in `ULI-78/build/bin`.
