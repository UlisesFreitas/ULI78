...
static void SDL_HapticLoadAxesList(SDL_Haptic_VIDPID_Naxes **entries, int *num_entries)
{
    ...
    spot = SDL_GetHint(SDL_HINT_JOYSTICK_HAPULI_AXES);
    if (!spot)
        return;
...
#if defined(SDL_HAPULI_DINPUT) || defined(SDL_HAPULI_XINPUT)
...
SDL_Haptic *SDL_HapticOpen(int device_index)
{
    ...
    if (haptic->supported & SDL_HAPULI_GAIN) {
        SDL_HapticSetGain(haptic, 100);
    }
    if (haptic->supported & SDL_HAPULI_AUTOCENTER) {
...
int SDL_HapticGetEffectStatus(SDL_Haptic *haptic, int effect)
{
    ...
    if (!(haptic->supported & SDL_HAPULI_STATUS)) {
...
int SDL_HapticSetGain(SDL_Haptic *haptic, int gain)
{
    ...
    env = SDL_getenv("SDL_HAPULI_GAIN_MAX");
    if (env) {
        ...
        /* We'll scale it linearly with SDL_HAPULI_GAIN_MAX */
...
int SDL_HapticPause(SDL_Haptic *haptic)
{
    ...
    if (!(haptic->supported & SDL_HAPULI_PAUSE)) {
...
int SDL_HapticRumbleSupported(SDL_Haptic *haptic)
{
    ...
    return (haptic->supported & (SDL_HAPULI_SINE | SDL_HAPULI_LEFTRIGHT)) != 0;
}
...
int SDL_HapticRumbleInit(SDL_Haptic *haptic)
{
    ...
    SDL_zerop(efx);
    if (haptic->supported & SDL_HAPULI_SINE) {
        efx->type = SDL_HAPULI_SINE;
        efx->periodic.direction.type = SDL_HAPULI_CARTESIAN;
        efx->periodic.period = 1000;
        efx->periodic.magnitude = 0x4000;
        efx->periodic.length = 5000;
        efx->periodic.attack_length = 0;
        efx->periodic.fade_length = 0;
    } else if (haptic->supported & SDL_HAPULI_LEFTRIGHT) { /* XInput? */
        efx->type = SDL_HAPULI_LEFTRIGHT;
...
int SDL_HapticRumblePlay(SDL_Haptic *haptic, float strength, Uint32 length)
{
    ...
    if (efx->type == SDL_HAPULI_SINE) {
        efx->periodic.magnitude = magnitude;
        efx->periodic.length = length;
    } else if (efx->type == SDL_HAPULI_LEFTRIGHT) {
