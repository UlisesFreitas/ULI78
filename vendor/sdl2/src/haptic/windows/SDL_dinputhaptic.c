...
static BOOL CALLBACK DI_EffectCallback(LPCDIEFFECTINFO pei, LPVOID pv)
{
    ...
    /* Get supported. */
    EFFECT_TEST(GUID_Spring, SDL_HAPULI_SPRING);
    EFFECT_TEST(GUID_Damper, SDL_HAPULI_DAMPER);
    EFFECT_TEST(GUID_Inertia, SDL_HAPULI_INERTIA);
    EFFECT_TEST(GUID_Friction, SDL_HAPULI_FRICTION);
    EFFECT_TEST(GUID_ConstantForce, SDL_HAPULI_CONSTANT);
    EFFECT_TEST(GUID_CustomForce, SDL_HAPULI_CUSTOM);
    EFFECT_TEST(GUID_Sine, SDL_HAPULI_SINE);
    /* !!! FIXME: put this back when we have more bits in 2.1 */
    /* EFFECT_TEST(GUID_Square, SDL_HAPULI_SQUARE); */
    EFFECT_TEST(GUID_Triangle, SDL_HAPULI_TRIANGLE);
    EFFECT_TEST(GUID_SawtoothUp, SDL_HAPULI_SAWTOOTHUP);
    EFFECT_TEST(GUID_SawtoothDown, SDL_HAPULI_SAWTOOTHDOWN);
    EFFECT_TEST(GUID_RampForce, SDL_HAPULI_RAMP);
...
static int SDL_DINPUT_HapticOpenFromDevice(SDL_Haptic *haptic, LPDIRECTINPUTDEVICE8 device8, SDL_bool is_joystick)
{
    ...
    if (!FAILED(ret)) { /* Gain is supported. */
        haptic->supported |= SDL_HAPULI_GAIN;
    }
    dipdw.diph.dwObj = 0;
    dipdw.diph.dwHow = DIPH_DEVICE;
    ...
    if (!FAILED(ret)) { /* Autocenter is supported. */
        haptic->supported |= SDL_HAPULI_AUTOCENTER;
    }

    /* Status is always supported. */
    haptic->supported |= SDL_HAPULI_STATUS | SDL_HAPULI_PAUSE;
                              ...
                              SDL_HapticEffect *src)
{
    ...
    /* Axes. */
    if (src->constant.direction.type == SDL_HAPULI_STEERING_AXIS) {
        dest->cAxes = 1;
    } else {
...
static void SDL_SYS_HapticFreeDIEFFECT(DIEFFECT *effect, int type)
{
    ...
    if (effect->lpvTypeSpecificParams) {
        if (type == SDL_HAPULI_CUSTOM) { /* Must free the custom data. */
...
int SDL_DINPUT_HapticRunEffect(SDL_Haptic *haptic, struct haptic_effect *effect, Uint32 iterations)
{
    ...
    if (iterations == SDL_HAPULI_INFINITY) {
        iter = INFINITE;
    } else {
        iter = iterations;
    }
