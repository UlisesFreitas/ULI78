...
static int SDLCALL SDL_RunXInputHaptic(void *arg)
{
    ...
    while (!SDL_AtomicGet(&hwdata->stopThread)) {
        ...
        if (hwdata->stopTicks) {
            if ((hwdata->stopTicks != SDL_HAPULI_INFINITY) && SDL_TICKS_PASSED(SDL_GetTicks(), hwdata->stopTicks)) {
...
static int SDL_XINPUT_HapticOpenFromUserIndex(SDL_Haptic *haptic, const Uint8 userid)
{
    ...
    haptic->supported = SDL_HAPULI_LEFTRIGHT;
...
{
    SDL_assert(base->type == SDL_HAPULI_LEFTRIGHT); /* should catch this at higher level */
...
int SDL_XINPUT_HapticUpdateEffect(SDL_Haptic *haptic, struct haptic_effect *effect, SDL_HapticEffect *data)
{
    ...
    SDL_assert(data->type == SDL_HAPULI_LEFTRIGHT);
...
{
    XINPUT_VIBRATION *vib = &effect->hweffect->vibration;
    SDL_assert(effect->effect.type == SDL_HAPULI_LEFTRIGHT); /* should catch this at higher level */
    SDL_LockMutex(haptic->hwdata->mutex);
    if (effect->effect.leftright.length == SDL_HAPULI_INFINITY || iterations == SDL_HAPULI_INFINITY) {
        haptic->hwdata->stopTicks = SDL_HAPULI_INFINITY;
        ...
        if ((haptic->hwdata->stopTicks == SDL_HAPULI_INFINITY) || (haptic->hwdata->stopTicks == 0)) {
