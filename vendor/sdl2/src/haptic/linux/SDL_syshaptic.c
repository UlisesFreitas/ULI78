...
static int EV_IsHaptic(int fd)
{
    ...
    EV_TEST(FF_CONSTANT, SDL_HAPULI_CONSTANT);
    EV_TEST(FF_SINE, SDL_HAPULI_SINE);
    /* !!! FIXME: put this back when we have more bits in 2.1 */
    /* EV_TEST(FF_SQUARE, SDL_HAPULI_SQUARE); */
    EV_TEST(FF_TRIANGLE, SDL_HAPULI_TRIANGLE);
    EV_TEST(FF_SAW_UP, SDL_HAPULI_SAWTOOTHUP);
    EV_TEST(FF_SAW_DOWN, SDL_HAPULI_SAWTOOTHDOWN);
    EV_TEST(FF_RAMP, SDL_HAPULI_RAMP);
    EV_TEST(FF_SPRING, SDL_HAPULI_SPRING);
    EV_TEST(FF_FRICTION, SDL_HAPULI_FRICTION);
    EV_TEST(FF_DAMPER, SDL_HAPULI_DAMPER);
    EV_TEST(FF_INERTIA, SDL_HAPULI_INERTIA);
    EV_TEST(FF_CUSTOM, SDL_HAPULI_CUSTOM);
    EV_TEST(FF_GAIN, SDL_HAPULI_GAIN);
    EV_TEST(FF_AUTOCENTER, SDL_HAPULI_AUTOCENTER);
    EV_TEST(FF_RUMBLE, SDL_HAPULI_LEFTRIGHT);
...
static int SDL_SYS_ToDirection(Uint16 *dest, SDL_HapticDirection *src)
{
    ...
    case SDL_HAPULI_SPHERICAL:
        /*
            ...
            --> add 9000
            --> finally convert to [0,0xFFFF] as in case SDL_HAPULI_POLAR.
        */
                    ...
                    /*
                      SDL_atan2 takes the parameters: Y-axis-value and X-axis-value (in that order)
                       ...
                       - X-axis-value is the first coordinate (from center to EAST)
                        ...
                        SDL_HAPULI_SPHERICAL and add another 9000 to get the polar value.
                      --> add 45000 in total
                      --> finally convert to [0,0xFFFF] as in case SDL_HAPULI_POLAR.
                    */
...
static int SDL_SYS_ToFFEffect(struct ff_effect *dest, SDL_HapticEffect *src)
{
    ...
    case SDL_HAPULI_CONSTANT:
        ...
        /* Replay */
        dest->replay.length = (constant->length == SDL_HAPULI_INFINITY) ? 0 : CLAMP(constant->length);
    ...
    case SDL_HAPULI_SINE:
    /* !!! FIXME: put this back when we have more bits in 2.1 */
    /* case SDL_HAPULI_SQUARE: */
    case SDL_HAPULI_TRIANGLE:
    case SDL_HAPULI_SAWTOOTHUP:
    case SDL_HAPULI_SAWTOOTHDOWN:
        ...
        /* Replay */
        dest->replay.length = (periodic->length == SDL_HAPULI_INFINITY) ? 0 : CLAMP(periodic->length);
        ...
        /* Periodic */
        if (periodic->type == SDL_HAPULI_SINE) {
            dest->u.periodic.waveform = FF_SINE;
            /* !!! FIXME: put this back when we have more bits in 2.1 */
            /* else if (periodic->type == SDL_HAPULI_SQUARE)
                dest->u.periodic.waveform = FF_SQUARE; */
        } else if (periodic->type == SDL_HAPULI_TRIANGLE) {
            dest->u.periodic.waveform = FF_TRIANGLE;
        } else if (periodic->type == SDL_HAPULI_SAWTOOTHUP) {
            dest->u.periodic.waveform = FF_SAW_UP;
        } else if (periodic->type == SDL_HAPULI_SAWTOOTHDOWN) {
    ...
    case SDL_HAPULI_SPRING:
    case SDL_HAPULI_DAMPER:
    case SDL_HAPULI_INERTIA:
    case SDL_HAPULI_FRICTION:
        condition = &src->condition;

        /* Header */
        if (condition->type == SDL_HAPULI_SPRING) {
            dest->type = FF_SPRING;
        } else if (condition->type == SDL_HAPULI_DAMPER) {
            dest->type = FF_DAMPER;
        } else if (condition->type == SDL_HAPULI_INERTIA) {
            dest->type = FF_INERTIA;
        } else if (condition->type == SDL_HAPULI_FRICTION) {
        ...
        /* Replay */
        dest->replay.length = (condition->length == SDL_HAPULI_INFINITY) ? 0 : CLAMP(condition->length);
    ...
    case SDL_HAPULI_RAMP:
        ...
        /* Replay */
        dest->replay.length = (ramp->length == SDL_HAPULI_INFINITY) ? 0 : CLAMP(ramp->length);
    ...
    case SDL_HAPULI_LEFTRIGHT:
        ...
        /* Replay */
        dest->replay.length = (leftright->length == SDL_HAPULI_INFINITY) ? 0 : CLAMP(leftright->length);
