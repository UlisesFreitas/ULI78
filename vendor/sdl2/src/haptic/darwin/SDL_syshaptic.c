...
static unsigned int GetSupportedFeatures(SDL_Haptic *haptic)
{
    ...
    /* Test for effects. */
    FF_TEST(FFCAP_ET_CONSTANTFORCE, SDL_HAPULI_CONSTANT);
    FF_TEST(FFCAP_ET_RAMPFORCE, SDL_HAPULI_RAMP);
    /* !!! FIXME: put this back when we have more bits in 2.1 */
    /* FF_TEST(FFCAP_ET_SQUARE, SDL_HAPULI_SQUARE); */
    FF_TEST(FFCAP_ET_SINE, SDL_HAPULI_SINE);
    FF_TEST(FFCAP_ET_TRIANGLE, SDL_HAPULI_TRIANGLE);
    FF_TEST(FFCAP_ET_SAWTOOTHUP, SDL_HAPULI_SAWTOOTHUP);
    FF_TEST(FFCAP_ET_SAWTOOTHDOWN, SDL_HAPULI_SAWTOOTHDOWN);
    FF_TEST(FFCAP_ET_SPRING, SDL_HAPULI_SPRING);
    FF_TEST(FFCAP_ET_DAMPER, SDL_HAPULI_DAMPER);
    FF_TEST(FFCAP_ET_INERTIA, SDL_HAPULI_INERTIA);
    FF_TEST(FFCAP_ET_FRICTION, SDL_HAPULI_FRICTION);
    FF_TEST(FFCAP_ET_CUSTOMFORCE, SDL_HAPULI_CUSTOM);

    /* Check if supports gain. */
    ret = FFDeviceGetForceFeedbackProperty(device, FFPROP_FFGAIN,
                                           &val, sizeof(val));
    if (ret == FF_OK) {
        supported |= SDL_HAPULI_GAIN;
    ...
    ret = FFDeviceGetForceFeedbackProperty(device, FFPROP_AUTOCENTER,
                                           &val, sizeof(val));
    if (ret == FF_OK) {
        supported |= SDL_HAPULI_AUTOCENTER;
    } else if (ret != FFERR_UNSUPPORTED) {
    ...
    supported |= SDL_HAPULI_STATUS | SDL_HAPULI_PAUSE;
