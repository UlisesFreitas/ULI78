...
static int prepare_colorindex(py_Ref index, u8* buffer)
{
    ...
    else if (py_istype(index, tp_list))
    {
        ...
        if (list_len > ULI_PALETTE_SIZE)
        {
            list_len = ULI_PALETTE_SIZE;
        }
...
static bool py_btn(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u32 pressed = core->api.btn((uli_mem*)core, button_id & 0x1f);
    py_newbool(py_retval(), pressed);
    return true;
}

// btnp(id: int, hold=-1, period=-1) -> bool
// u32 (*btnp)(uli_mem*, s32, s32, s32)
static bool py_btnp(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u32 pressed = core->api.btnp((uli_mem*)core, button_id, hold, period);
    py_newbool(py_retval(), pressed);
    return true;
}
...
static bool py_cls(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.cls((uli_mem*)core, color);
    py_newnone(py_retval());
    return true;
}

// spr(id: int, x: int, y: int, colorkey=-1, scale=1, flip=0, rotate=0, w=1, h=1)
// void (*spr)(uli_mem*, s32, s32, s32, s32, s32, u8*, u8, s32, uli_flip, uli_rotate)
static bool py_spr(int argc, py_Ref argv)
{
    ...
    u8 colors[ULI_PALETTE_SIZE];
    int colors_count = prepare_colorindex(py_arg(3), colors);
    if (colors_count == -1) return false;

    s32 scale = py_toint(py_arg(4));
    uli_flip flip = py_toint(py_arg(5));
    uli_rotate rotate = py_toint(py_arg(6));
    s32 w = py_toint(py_arg(7));
    s32 h = py_toint(py_arg(8));

    uli_core* core = get_core();
    core->api.spr((uli_mem*)core, spr_id, x, y, w, h, colors, colors_count, scale, flip, rotate);
    py_newnone(py_retval());
    return true;
}

// print(text, x=0, y=0, color=15, fixed=False, scale=1, alt=False)
// s32 (*print)(uli_mem*, const char*, s32, s32, u8, bool, s32, bool)
static bool py_print(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    s32 ps = core->api.print((uli_mem*)core, text, x, y, color, fixed, scale, alt);
    py_newint(py_retval(), ps);
    return true;
}

// circ(x: int, y: int, radius: int, color: int)
// void (*circ)(uli_mem*, s32, s32, s32, u8)
static bool py_circ(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.circ((uli_mem*)core, x, y, radius, color);
    py_newnone(py_retval());
    return true;
}

// circb(x: int, y: int, radius: int, color: int)
// void (*circb)(uli_mem*, s32, s32, s32, u8)
static bool py_circb(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.circb((uli_mem*)core, x, y, radius, color);
    py_newnone(py_retval());
    return true;
}

// clip(x: int, y: int, width: int, height: int)
// void (*clip)(uli_mem*, s32, s32, s32, s32)
static bool py_clip(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.clip((uli_mem*)core, x, y, width, height);
    py_newnone(py_retval());
    return true;
}

// elli(x: int, y: int, a: int, b: int, color: int)
// void (*elli)(uli_mem*, s32, s32, s32, s32, u8)
static bool py_elli(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.elli((uli_mem*)core, x, y, a, b, color);
    py_newnone(py_retval());
    return true;
}

// ellib(x: int, y: int, a: int, b: int, color: int)
// void (*ellib)(uli_mem*, s32, s32, s32, s32, u8)
static bool py_ellib(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.ellib((uli_mem*)core, x, y, a, b, color);
    py_newnone(py_retval());
    return true;
}
...
{
    uli_core* core = get_core();
    core->api.exit((uli_mem*)core);
    py_newnone(py_retval());
    return true;
}

// fget(sprite_id: int, flag: int) -> bool
// bool (*fget)(uli_mem*, s32, u8)
static bool py_fget(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    bool res = core->api.fget((uli_mem*)core, sprite_id, flag);
    py_newbool(py_retval(), res);
    return true;
}

// fset(sprite_id: int, flag: int, b: bool)
// void (*fset)(uli_mem*, s32, u8, bool)
static bool py_fset(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.fset((uli_mem*)core, sprite_id, flag, b);
    py_newnone(py_retval());
    return true;
}

// font(text: str, x: int, y: int, chromakey: int, char_width=8, char_height=8, fixed=False, scale=1, alt=False) -> int
// s32 (*font)(uli_mem*, const char*, s32, s32, u8*, u8, s32, s32, bool, s32, bool)
static bool py_font(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    s32 res = core->api.font((uli_mem*)core, text, x, y, &chromakey,
...
// key(code=-1) -> bool
// bool (*key)(uli_mem*, uli_key)
static bool py_key(int argc, py_Ref argv)
{
    PY_CHECK_ARG_TYPE(0, tp_int);
    s32 code = py_toint(py_arg(0));

    if (code <= -1 || code >= uli_keys_count)
    {
        code = uli_key_unknown;
    }

    uli_core* core = get_core();
    bool pressed = core->api.key((uli_mem*)core, (uli_key)code);
    py_newbool(py_retval(), pressed);
    return true;
}

// keyp(code=-1, hold=-1, period=-17) -> int
// bool (*keyp)(uli_mem*, uli_key, s32, s32)
static bool py_keyp(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    bool pressed = core->api.keyp((uli_mem*)core, (uli_key)code, hold, period);
    py_newbool(py_retval(), pressed);
    return true;
}

// line(x0: float, y0: float, x1: float, y1: float, color: int)
// void (*line)(uli_mem*, float, float, float, float, u8)
static bool py_line(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.line((uli_mem*)core, x0, y0, x1, y1, color);
    py_newnone(py_retval());
    return true;
}
...
// void (*map)(uli_mem*, s32, s32, s32, s32, s32, s32, u8*, u8, s32, RemapFunc, void*)
static bool py_map(int argc, py_Ref argv)
{
    ...
    {
        core->api.map((uli_mem*)core, x, y, w, h, sx, sy, colors, colors_count, scale, remap_callback, py_arg(8));
    }
    else
    {
        core->api.map((uli_mem*)core, x, y, w, h, sx, sy, colors, colors_count, scale, NULL, NULL);
    }
    py_newnone(py_retval());
    return true;
}
...
// void (*memcpy)(uli_mem*, s32, s32, s32)
static bool py_memcpy(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.memcpy((uli_mem*)core, dest, source, size);
    py_newnone(py_retval());
    return true;
}

// memset(dest: int, value: int, size: int)
// void (*memset)(uli_mem*, s32, u8, s32)
static bool py_memset(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.memset((uli_mem*)core, dest, value, size);
    py_newnone(py_retval());
    return true;
}

// mget(x: int, y: int) -> int
// u8 (*mget)(uli_mem*, s32, s32)
static bool py_mget(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u8 res = core->api.mget((uli_mem*)core, x, y);
    py_newint(py_retval(), res);
    return true;
}

// mset(x: int, y: int, tile_id: int)
// void (*mset)(uli_mem*, s32, s32, u8)
static bool py_mset(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.mset((uli_mem*)core, x, y, tile_id);
    py_newnone(py_retval());
    return true;
}
...
// void (*music)(uli_mem*, s32, s32, s32, bool, bool, s32, s32)
static bool py_music(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.music((uli_mem*)core, -1, 0, 0, false, false, -1, -1);
    core->api.music((uli_mem*)core, track, frame, row, loop, sustain, tempo, speed);
    py_newnone(py_retval());
    return true;
}

// peek(addr: int, bits=8) -> int
// u8 (*peek)(uli_mem*, s32, s32)
static bool py_peekbits(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u8 res = core->api.peek((uli_mem*)core, addr, bits);
    py_newint(py_retval(), res);
    return true;
}

// peek1(addr: int) -> int
// u8 (*peek1)(uli_mem*, s32)
static bool py_peek1(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u8 res = core->api.peek1((uli_mem*)core, addr);
    py_newint(py_retval(), res);
    return true;
}

// peek2(addr: int) -> int
// u8 (*peek2)(uli_mem*, s32)
static bool py_peek2(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u8 res = core->api.peek2((uli_mem*)core, addr);
    py_newint(py_retval(), res);
    return true;
}

// peek4(addr: int) -> int
// u8 (*peek4)(uli_mem*, s32)
static bool py_peek4(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    u8 res = core->api.peek4((uli_mem*)core, addr);
    py_newint(py_retval(), res);
    return true;
}

// pix(x: int, y: int, color: int | None = None) -> int | None
// u8 (*pix)(uli_mem*, s32, s32, u8, bool)
static bool py_pix(int argc, py_Ref argv)
{
    ...
    if (!py_isnone(py_arg(2)))
    {
        ...
        core->api.pix((uli_mem*)core, x, y, color, false);
        py_newnone(py_retval());
    }
    else
    {
        // get pixel
        u8 res = core->api.pix((uli_mem*)core, x, y, 0, true);
        py_newint(py_retval(), res);
    }
    return true;
}
...
static bool py_pmem(int argc, py_Ref argv)
{
    ...
    {
        return ValueError("invalid uli persistent index");
    }
    ...
    if (!py_isnone(py_arg(1)))
    {
        ...
        core->api.pmem((uli_mem*)core, index, value, false);
        py_newnone(py_retval());
    }
    else
    {
        // get persistent memory
        u32 res = core->api.pmem((uli_mem*)core, index, 0, true);
        py_newint(py_retval(), res);
    }
    return true;
}
...
static bool py_pokebits(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.poke((uli_mem*)core, addr, value, bits);
    py_newnone(py_retval());
    return true;
}

// poke1(addr: int, value: int)
// void (*poke1)(uli_mem*, s32, u8)
static bool py_poke1(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.poke1((uli_mem*)core, addr, value);
    py_newnone(py_retval());
    return true;
}

// poke2(addr: int, value: int)
// void (*poke2)(uli_mem*, s32, u16)
static bool py_poke2(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.poke2((uli_mem*)core, addr, value);
    py_newnone(py_retval());
    return true;
}

// poke4(addr: int, value: int)
// void (*poke4)(uli_mem*, s32, u32)
static bool py_poke4(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.poke4((uli_mem*)core, addr, value);
    py_newnone(py_retval());
    return true;
}

// rect(x: int, y: int, w: int, h: int, color: int)
// void (*rect)(uli_mem*, s32, s32, s32, s32, u8)
static bool py_rect(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.rect((uli_mem*)core, x, y, w, h, color);
    py_newnone(py_retval());
    return true;
}

// rectb(x: int, y: int, w: int, h: int, color: int)
// void (*rectb)(uli_mem*, s32, s32, s32, s32, u8)
static bool py_rectb(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.rectb((uli_mem*)core, x, y, w, h, color);
    py_newnone(py_retval());
    return true;
}
...
{
    uli_core* core = get_core();
    core->api.reset((uli_mem*)core);
    py_newnone(py_retval());
    return true;
}

// sfx(id: int, note=-1, duration=-1, channel=0, volume=15, speed=0)
// void (*sfx)(uli_mem*, s32, s32, s32, s32, s32, s32, s32, s32)
static bool py_sfx(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.sfx((uli_mem*)core, id, note, octave, duration, channel, volume & 0xf, volume & 0xf, speed);
    py_newnone(py_retval());
    return true;
}

// sync(mask=0, bank=0, tocart=False)
// void (*sync)(uli_mem*, u32, s32, bool)
static bool py_sync(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.sync((uli_mem*)core, mask, bank, tocart);
    py_newnone(py_retval());
    return true;
}
...
// void (*ttri)(uli_mem*, float, float, float, float, float, float, float, float, float, float, float, float, uli_texture_src, u8*, s32, float, float, float, bool)
static bool py_ttri(int argc, py_Ref argv)
{
    ...
    uli_texture_src texsrc = py_toint(py_arg(12));
...
{
    uli_core* core = get_core();
    double res = core->api.time((uli_mem*)core);
    py_newfloat(py_retval(), res);
    return true;
}

// trace(message, color=15)
// void (*trace)(uli_mem*, const char*, u8)
static bool py_trace(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.trace((uli_mem*)core, msg, color);
    py_pop();
    py_newnone(py_retval());
    return true;
}
...
// void (*tri)(uli_mem*, float, float, float, float, float, float, u8)
static bool py_tri(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.tri((uli_mem*)core, x1, y1, x2, y2, x3, y3, color);
    py_newnone(py_retval());
    return true;
}

// trib(x1: float, y1: float, x2: float, y2: float, x3: float, y3: float, color: int)
// void (*trib)(uli_mem*, float, float, float, float, float, float, u8)
static bool py_trib(int argc, py_Ref argv)
{
    ...
    uli_core* core = get_core();
    core->api.trib((uli_mem*)core, x1, y1, x2, y2, x3, y3, color);
    py_newnone(py_retval());
    return true;
}
...
{
    uli_core* core = get_core();
    s32 res = core->api.tstamp((uli_mem*)core);
    py_newint(py_retval(), res);
    return true;
}

// vbank(bank: int | None = None) -> int
// s32 (*vbank)(uli_mem*, s32)
static bool py_vbank(int argc, py_Ref argv)
{
    ...
    if (bank >= 0)
    {
        core->api.vbank((uli_mem*)core, bank);
    }
...
void close_pkpy_v2(uli_mem* uli)
{
...
static bool init_pkpy_v2(uli_mem* uli, const char* code)
{
    py_initialize();
    uli_core* core = (uli_core*)uli;
    close_pkpy_v2(uli);
...
void tick_pkpy_v2(uli_mem* uli)
{
    uli_core* core = (uli_core*)uli;
    if (!core->currentVM) return; //no vm

    py_GlobalRef py_tick = py_getglobal(N.ULI);
    if (!py_tick) return;
...
void callback_scanline(uli_mem* uli, s32 row, void* data)
{
    uli_core* core = (uli_core*)uli;
    if (!core->currentVM) return; //no vm
...
void callback_border(uli_mem* uli, s32 row, void* data)
{
    uli_core* core = (uli_core*)uli;
    if (!core->currentVM) return; //no vm
...
void callback_menu(uli_mem* uli, s32 index, void* data)
{
    uli_core* core = (uli_core*)uli;
    if (!core->currentVM) return; //no vm
...
static const u8 DemoRom[] =
    {
#include "../build/assets/pythondemo.uli.dat"
};

static const u8 MarkRom[] =
    {
#include "../build/assets/pythonmark.uli.dat"
};

ULI_EXPORT const uli_script EXPORT_SCRIPT(Python) =
    {
        .id = 20,
        .name = "python",
        .fileExtension = ".py",
        ...
        .mark = {MarkRom, sizeof MarkRom, "pythonmark.uli"},
};
